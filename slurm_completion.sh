...
function _switches() {
echo $(scontrol show topology  | grep -P -o '(?<=SwitchName=)[A-Za-z0-9]* ')
}
function _jobs() {
...
_scontrol()
{
    ...
    local commands="abort checkpoint cluster create completing delete details\
		    ...
		    pidinfo listpids ping quit reboot reconfigure release\
		    requeue requeuehold schedloglevel resume schedloglevel\
		    setdebug setdebugflags show shutdown suspend\
		    takeover uhold update verbose version wait_job\
            fsdampeningfactor top write"
	...
	if param "batch_script"          ; then offer  "$(_jobs)"         ; fi
	...
	objects="aliases config block daemons frontend hostlist hostlistsorted\
		 ...
		 submp topology assoc_mgr cache federation burstbuffer powercap"
	...
	if param "topology"     ; then offer_list "$(_nodes) $(_switches)" ; fi
	if param "partitions"   ; then offer "$(_partitions)"   ; fi
	if param "reservations" ; then offer "$(_reservations)"  ; fi
	if param "assoc_mgr"    ; then offer "users= account= qos= flags="  ; fi
	if param "cache"        ; then offer "users= accounts= qos= flags="  ; fi
	if param "users"        ; then offer_many "$(_users)"  ; fi
	if param "accounts"        ; then offer_many "$(_accounts)"  ; fi
	if param "qos"        ; then offer_many "$(_qos)"  ; fi
	if param "flags"        ; then offer_list "users assoc qos"  ; fi
	#TODO if object "steps"
	;;
